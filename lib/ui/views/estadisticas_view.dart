...
class EstadisticasView extends StatelessWidget {
          ...
          builder: (context, vm, child) {
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  // 1. EL MENÚ DESPLEGABLE (DROPDOWN) ACTUALIZADO
                  const Text(
                  ...
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<ModoEstadistica>(
                        value: vm.modoActual,
                        isExpanded: true,
                        icon: const Icon(Icons.keyboard_arrow_down, color: kOrange),
                        items: const [
                          DropdownMenuItem(
                            value: ModoEstadistica.solicitudesCarrera, 
                            child: Text('📊 Porcentaje de Solicitudes por Carrera')
                          ),
                          DropdownMenuItem(
                            value: ModoEstadistica.solicitudesMateria, 
                            child: Text('📊 Porcentaje de Solicitudes por Materia')
                          ),
                          DropdownMenuItem(
                            value: ModoEstadistica.calificacionMateria, 
                            child: Text('⭐ Mejores Calificados por Materia (Top 5)')
                          ),
                          DropdownMenuItem(
                            value: ModoEstadistica.calificacionMaterial, 
                            child: Text('⭐ Mejores Libros Calificados (Top 5)')
                          ),
                        ],
                        onChanged: (ModoEstadistica? nuevoModo) {
                          if (nuevoModo != null) vm.cambiarModo(nuevoModo);
                        },
                      ),
                    ),
                  ),
                  ...
                  Expanded(
                    child: vm.isLoading
                        ...
                        : vm.datosProcesados.isEmpty
                            ? const Center(child: Text('No hay datos suficientes para mostrar.'))
  ...
  Widget _construirGraficoYLeyenda(EstadisticasViewModel vm) {
    ...
    bool esModoSolicitudes = vm.modoActual == ModoEstadistica.solicitudesCarrera || 
                             vm.modoActual == ModoEstadistica.solicitudesMateria;

    // Paleta de colores para los gráficos
    List<Color> colores = [kOrange, Colors.blue, Colors.green, Colors.purple, Colors.red, Colors.grey];
    ...
    return Column(
      children: [
        ...
        Expanded(
          flex: 2,
          child: _construirLeyenda(vm.datosProcesados, colores, esModoSolicitudes),
        ),
      ],
    );
  }
            ...
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (double value, TitleMeta meta) {
                ...
                return Padding(
                  ...
                  child: Text(nombre.length > 10 ? '${nombre.substring(0, 8)}...' : nombre, 
  ...
  Widget _construirLeyenda(Map<String, double> datos, List<Color> colores, bool esPorcentaje) {
        ...
        return Padding(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(clave, style: const TextStyle(fontSize: 16, color: kDarkGray, fontWeight: FontWeight.w500)),
              ),
              Text(
